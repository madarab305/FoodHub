...
def like(request, restaraunt_id):
    restaraunt_obj = Restaurant.objects.get(id=restaraunt_id)

    like_obj, created = FavourateRestaurant.objects.get_or_create(user=request.user, restaurant=restaraunt_obj)
    ...
    like_count = restaraunt_obj.favouraterestaurant_set.all().count()
